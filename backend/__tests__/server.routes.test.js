...
describe('server routes (white-box)', () => {
  ...
  beforeEach(() => {
    ...
    jest.mock('../lib/logger.js', () => ({ logger: { info: jest.fn() }, logError: jest.fn() }));
    jest.mock('../lib/errorHandler.js', () => ({ errorHandler: (err, req, res, next) => next(err), handleRouteError: jest.fn(), notFoundHandler: (req, res, next) => next() }));
    jest.mock('../lib/db.js', () => ({ pool: { query: jest.fn().mockResolvedValue([[]]) } }));
    ...
    jest.mock('../lib/mailer.js', () => ({ sendPasswordResetEmail: jest.fn().mockResolvedValue({ previewUrl: null }) }));
    jest.mock('../lib/middleware.js', () => ({ requireLogin: (req, res, next) => next(), requireRole: () => ((req, res, next) => next()) }));
    jest.mock('../lib/security.js', () => ({ isStrongPassword: jest.fn() }));
    jest.mock('../lib/searchService.js', () => ({ getResourceBySlug: jest.fn(), getSearchMeta: jest.fn().mockReturnValue({}), searchResources: jest.fn().mockReturnValue([]) }));

    // Finally require the server module (it will configure routes on fakeApp)
    // Use require with the actual relative path to server.js
    require('../server.js');
  });
  ...
  test('POST /api/auth/register - validates input and success path (white-box)', async () => {
    const { findUserByEmail, createUser, createSessionForUser, sanitizeUser } = require('../lib/auth.js');
    const { isStrongPassword } = require('../lib/security.js');
